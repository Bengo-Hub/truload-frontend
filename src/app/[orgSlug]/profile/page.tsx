          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <TwoFactorCodeInput
                onSubmit={async (code) => {
                  try {
                    const result = await enable2FA.mutateAsync({ verificationCode: code });
                    if (result.recoveryCodes?.length) {
                      setRecoveryCodes(result.recoveryCodes);
                    }
                    setStep('idle');
                    toast.success('Two-factor authentication enabled');
                    useAuthStore.getState().clearRequires2FASetup();
                  } catch {
        ...
        {requires2FASetup && (
          <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
            <p className="text-sm text-amber-800 flex items-center gap-2">
              ...
              Your organization requires two-factor authentication. You must enable 2FA in the Security tab below before you can access the rest of the app.
            </p>
          </div>
        )}
