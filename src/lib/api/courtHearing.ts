...
export async function getHearingById(id: string): Promise<CourtHearingDto> {
  const { data } = await apiClient.get<CourtHearingDto>(`/case/hearings/${id}`);
  return data;
}
...
export async function updateHearing(
  ...
  const { data } = await apiClient.put<CourtHearingDto>(`/case/hearings/${id}`, request);
  return data;
}
...
export async function adjournHearing(
  ...
  const { data } = await apiClient.post<CourtHearingDto>(`/case/hearings/${id}/adjourn`, request);
  return data;
}
...
export async function completeHearing(
  ...
  const { data } = await apiClient.post<CourtHearingDto>(`/case/hearings/${id}/complete`, request);
  return data;
}
...
export async function deleteHearing(id: string): Promise<void> {
  await apiClient.delete(`/case/hearings/${id}`);
}
...
export async function downloadCourtMinutesPdf(id: string): Promise<Blob> {
  const { data } = await apiClient.get<Blob>(`/case/hearings/${id}/minutes/pdf`, {
    responseType: 'blob',
  });
  return data;
}
